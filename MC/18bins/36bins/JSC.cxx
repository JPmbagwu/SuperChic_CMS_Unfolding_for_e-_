...
void ManuelElectronResponseMatrix() {
    ...
    string outfile = "JSCC36enhanced_superchic2018_dielectrons_full.root";
    ...
    cUnfoldDataOverAcceptance->SaveAs("JSC36UnfoldDataOverAcceptance.pdf");
    ...
    cDataUnfold->Update();
    cDataUnfold->SaveAs("JSC36bottom_line_test.pdf");
    ...
    cUnfoldVsGen->Update();
    cUnfoldVsGen->SaveAs("JSC36UnfoldVsGenTruth.pdf");
...
//        latex.DrawLatex(0.84, 0.20, "p_{T}_{e} > 0.0 GeV");
        ...
        c->Update();
        c->Draw();
        c->Print("JSC36SuperchicResponseMatrix.pdf");
        delete c;
    }
    ...
    // --- Added: Compute and print uncertainties on totals for before/after unfolding ---
    // Measured (pre-unfolding, purity-corrected data)
    double measured_integral = hDeltaPhiPurityCorrected->Integral();
    double measured_unc = 0.0;
    for (int i = 1; i <= hDeltaPhiPurityCorrected->GetNbinsX(); ++i) {
        measured_unc += pow(hDeltaPhiPurityCorrected->GetBinError(i), 2);
    }
    measured_unc = sqrt(measured_unc);

    // Unfolded data (post-unfolding)
    if (!UnfoldData) {
        cerr << "Error: UnfoldData histogram is null!" << endl;
        return;
    }
    double unfolded_integral = UnfoldData->Integral();
    double unfolded_unc = 0.0;

    // Compute uncertainty using bin errors (fallback)
    for (int i = 1; i <= UnfoldData->GetNbinsX(); ++i) {
        unfolded_unc += pow(UnfoldData->GetBinError(i), 2);
    }
    unfolded_unc = sqrt(unfolded_unc);
    cerr << "Warning: Covariance matrix not available in this RooUnfold version, using bin errors for uncertainty calculation." << endl;

    // Print results
    cout << "Pre-unfolding (measured) integral: " << measured_integral << " +/- " << measured_unc << endl;
    cout << "Post-unfolding integral: " << unfolded_integral << " +/- " << unfolded_unc << endl;
