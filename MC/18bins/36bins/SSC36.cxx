...
void ManuelElectronResponseMatrix() {
    ...
    string outfile = "SSC36enhanced_superchic2018_dielectrons_full.root";
    ...
    TFile *dataFile = new TFile("/afs/cern.ch/user/j/jmbagwu/NEWSC/e-/36bins/delta_phi_histogram_electron_nozdc02.root", "READ");
    ...
    for (int j = 0; j < numberentry; j++) {
        ...
        // Compute DeltaPhi using the electron (GLep1, e-)
        float GenDPx = GDiele.Pt() * cos(GDiele.Phi());
        float GenDPy = GDiele.Pt() * sin(GDiele.Phi());
        float GenMPx = GLep1.Pt() * cos(GLep1.Phi()); // Use GLep1 (electron)
        ...
        if (Twoelectron && Twotrk) {
            ...
            // Vertex selection - use reco_trkvz
            if (reco_trkvz->size() == 0) continue;
            
            // Assign electrons based on charge
            int EleP = ((*reco_eleCharge)[0] == 1) ? 0 : 1;  // Positron (e+)
            int EleN = 1 - EleP;                             // Electron (e-)
            iRp = EleP;
            iRn = EleN;
            ...
            // Compute DeltaPhi using the electron (RLep2, e-)
            float RecoDPx = RDiele.Pt() * cos(RDiele.Phi());
            float RecoDPy = RDiele.Pt() * sin(RDiele.Phi());
            float RecoMPx = RLep2.Pt() * cos(RLep2.Phi()); // Use RLep2 (electron)
            float RecoMPy = RLep2.Pt() * sin(RLep2.Phi());
        ...
        if (DEBUG && TrueEvt) {
            ...
            if (iRn >= 0) {
                cout << "R lep1 charge (positron) is " << reco_eleCharge->at(iRp) << endl;
                cout << "R lep2 charge (electron) is " << reco_eleCharge->at(iRn) << endl;
                cout << "R lep2 phi (electron) is " << RLep2.Phi() << endl;
                cout << "RDielectron phi is " << RDiele.Phi() << endl;
            }
            cout << "G lep1 phi (electron) is " << GLep1.Phi() << endl;
        ...
        if (DEBUG && TrueEvt) {
            ...
            cout << "Reco: positron idx=" << iRp << " charge=" << reco_eleCharge->at(iRp) << endl;
            cout << "Reco: electron idx=" << iRn << " charge=" << reco_eleCharge->at(iRn) << endl;
    ...
    // --- Calculate total statistical uncertainty before unfolding ---
    double total_unc_before = 0.0;
    for (int i = 1; i <= hDeltaPhiPurityCorrected->GetNbinsX(); ++i) {
        total_unc_before += pow(hDeltaPhiPurityCorrected->GetBinError(i), 2);
    }
    total_unc_before = sqrt(total_unc_before);
    cout << "Total statistical uncertainty before unfolding (hDeltaPhiPurityCorrected): " << total_unc_before << endl;
    ...
    // --- Calculate total statistical uncertainty after unfolding ---
    double total_unc_after = 0.0;
    for (int i = 1; i <= UnfoldData->GetNbinsX(); ++i) {
        total_unc_after += pow(UnfoldData->GetBinError(i), 2);
    }
    total_unc_after = sqrt(total_unc_after);
    cout << "Total statistical uncertainty after unfolding (UnfoldData, before scaling): " << total_unc_after << endl;

    // --- Scale uncertainties of UnfoldData to match hDeltaPhiPurityCorrected ---
    if (total_unc_after > 0) {
        double scale_factor = total_unc_before / total_unc_after; // Scale factor to match pre-unfolding uncertainty
        cout << "Scaling factor for UnfoldData uncertainties: " << scale_factor << endl;
        for (int i = 1; i <= UnfoldData->GetNbinsX(); ++i) {
            double current_error = UnfoldData->GetBinError(i);
            UnfoldData->SetBinError(i, current_error * scale_factor);
        }
        // Verify the new total uncertainty
        double new_total_unc_after = 0.0;
        for (int i = 1; i <= UnfoldData->GetNbinsX(); ++i) {
            new_total_unc_after += pow(UnfoldData->GetBinError(i), 2);
        }
        new_total_unc_after = sqrt(new_total_unc_after);
        cout << "Total statistical uncertainty after unfolding (UnfoldData, after scaling): " << new_total_unc_after << endl;
    } else {
        cerr << "Warning: Total uncertainty after unfolding is zero, cannot scale!" << endl;
    }
    ...
    latexUnfoldAcc.DrawLatex(0.84, 0.13, "7.0 < M_{ee} < 10.0 GeV");
    
    cUnfoldDataOverAcceptance->Update();
    cUnfoldDataOverAcceptance->SaveAs("SSC36UnfoldDataOverAcceptance.pdf");
    ...
    cDataUnfold->Update();
    cDataUnfold->SaveAs("SSC36bottom_line_test.pdf");
    ...
    cUnfoldVsGen->Update();
    cUnfoldVsGen->SaveAs("SSC36UnfoldVsGenTruth.pdf");
    ...
    if (hResponse) {
        ...
        latex.DrawLatex(0.84, 0.13, "7.0 < M_{ee} < 10.0 GeV");

        c->Update();
        c->Draw();
        c->Print("SSC36SuperchicResponseMatrix.pdf");
        delete c;
    }
